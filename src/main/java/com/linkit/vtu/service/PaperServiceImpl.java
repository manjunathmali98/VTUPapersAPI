...
public class PaperServiceImpl implements PaperService {
	...
	public List<Paper> addPaper(Paper paperObj) {
		...
		for (Subject sub : subjects) {
			...
			String fileName = String.join("_", scheme, shortCode, String.valueOf(sem), sub.getSubjectCode(),
					month.toLowerCase(), year);
			System.out.println("sub name is" + fileName);
			String fileUrl = String.join("/", scheme, shortCode, "SEM_" + String.valueOf(sem), sub_name, fileName)
					+ ".pdf";
		...
		/*
		 * Qpaper qp = new Qpaper(); qp.setPaper(paper); try {
		 * qp.setDataFile(file.getBytes()); } catch (IOException e) {
		 * System.out.println("Input File Exception"); e.printStackTrace(); }
		 * 
	...
	/*
	 * @Override public Paper addPaper(String Month, String Year, String
	 * subjectCode) {
	 * 
	 * Subject sub = subjectService.getBySubjectCode(subjectCode);
	 * 
	 * System.out.println(Month); System.out.println(subjectCode); Paper paperObj =
	 * new Paper(); paperObj.setSubject(sub); paperObj.setMonth(Month);
	 * paperObj.setYear(Year);
	 * 
	 * System.out.println(sub);
	 * 
	 * String scheme = sub.getSemester().getBranch().getScheme().getYear(); String
	 * shortCode = sub.getSemester().getBranch().getShortCode(); int sem =
	 * sub.getSemester().getSem(); String month = paperObj.getMonth(); String year =
	 * paperObj.getYear(); Stream<String> subArray =
	 * Arrays.asList(sub.getSubjectName().toLowerCase().split(" ")).stream(); String
	 * sub_name = subArray.collect(Collectors.joining("_"));
	 * 
	 * System.out.println("sub name is"+sub_name); String fileName =
	 * String.join("_", scheme, shortCode, String.valueOf(sem),
	 * sub.getSubjectCode(), month.toLowerCase(), year);
	 * System.out.println("sub name is"+fileName); String fileUrl = String.join("/",
	 * scheme, shortCode, "SEM_"+String.valueOf(sem), sub_name, fileName)+".pdf";
	 * System.out.println("file Url is"+ fileUrl); paperObj.setFileUrl(fileUrl);
	 * 
	 * Paper paper = pRepo.save(paperObj); return paper; }
	 */
